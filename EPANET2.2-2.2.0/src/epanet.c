...
*/
{
    ...
    *value = 0.0;
    ...
    case EN_PUMP_EFFIC:
        ...
        for(int i=0; i < 24; i++)
            value[i] = net->Pump[pump_idx].Schedule[i];
        break;
...
*/
{
    if((pump_index <= 0)||(pump_index > p->network.Npumps)) return(401);
...
*/
{
    ...
    if((tank_index <= 0)||(tank_index > p->network.Ntanks)) return(402);
...
*/
{
    ...
    if (p->network.Npumps == 0) {
        *cost = 0.0;
        return(0);
    }

    *cost = 0.0;
    for (j=1; j<=p->network.Npumps; j++)
        *cost += p->network.Pump[j].Energy.TotalCost;

    *cost +=  (p->hydraul.Emax * p->hydraul.Dcost);
...
*/
{  	
		int pumpindex = findpump(&p->network, index);
    if (index <= 0 || index > p->network.Nlinks) return(204);
    *value = p->network.Pump[pumpindex].Switches;
    return(0);
}

int DLLEXPORT EN_addleveltrigger(EN_Project p, int pump_index, int tank_index,
...
int DLLEXPORT EN_getnode_xcoord(EN_Project p, int index, float *x)
{
...
int DLLEXPORT EN_getnode_ycoord(EN_Project p, int index, float *y)
{
    if (!Openflag) return(102);
    if (index <= 0 || index > Nnodes) return(203);
