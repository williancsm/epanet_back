...
$LastChangedBy: manu $ $Revision: 62 $
$LastChangedDate: 2006-03-20 16:44:22 +0000 (Mon, 20 Mar 2006) $
            ...
            if (Htime == 0)
                sprintf(s1,FMT52,atime,LinkTxt[(int) Link[i].Type],Link[i].ID,
                        StatTxt[(int) S[i]]);
                         ...
                         StatTxt[(int) OldStat[i]],StatTxt[(int) S[i]]);
            writeline(s1);
            OldStat[i] = S[i];
        }
    }
  ...
  */
{
    ...
    case HILEVEL:
        n = Control[i].Node;
        sprintf(Msg,FMT54,clocktime(Atime,Htime),LinkTxt[(int) Link[k].Type],
                Link[k].ID,NodeTxt[getnodetype(n)],Node[n].ID);
        break;
    case TIMER:
    case TIMEOFDAY:
        sprintf(Msg,FMT55,clocktime(Atime,Htime),LinkTxt[(int) Link[k].Type],
                Link[k].ID);
        break;
    default: return;
    }
...
*/
{
    sprintf(Msg,FMT63,clocktime(Atime,Htime),LinkTxt[(int) Link[k].Type],
            Link[k].ID,ruleID);
    writeline(Msg);
}
...
*/
{
    ...
    for (i=1; i<=Nvalves; i++)
    {
        ...
        if (S[j] >= XFCV)
        {
            sprintf(Msg,WARN05,LinkTxt[(int) Link[j].Type],Link[j].ID,
                    StatTxt[(int) S[j]],clocktime(Atime,Htime));
            if (Messageflag) writeline(Msg);
            flag = 5;
            Nwarnings++;
        }
    ...
    for (i=1; i<=Npumps; i++)
    {
        ...
        {
            sprintf(Msg,WARN04,Link[j].ID,StatTxt[(int) S[j]],
