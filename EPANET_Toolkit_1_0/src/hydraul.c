...
*/
{
    ...
    *tstep = 0;
    ...
    if (time->Htime < time->Dur)
        ...
        for (i=1, tmp_float=0.0; i<=pr->network.Njuncs; i++) tmp_float += hyd->EmitterFlow[i];
        hyd->TotalSystemLeakage += tmp_float*hydstep;

        for (i=1, tmp_float=0.0; i<=pr->network.Ntanks; i++)
            if (pr->network.Tank[i].A == 0.0)
                tmp_float += hyd->NodeDemand[pr->network.Tank[i].Node];
...
*/
{
    int pump_idx = findpump(&pr->network, index);
    long Htime_ = pr->times.Htime;
    /* Htime == 0 is ignored because if the final status is the same
       as the initial status, there is no pump switch.  */
    if (Htime_ > 0 && status != pr->network.Link[index].Status) { 
        pr->network.Pump[pump_idx].Switches++; 
    }
    
    if (Htime_ < 24 * 3600 && pr->network.Pump[pump_idx].Schedule_idx < 24) {
        pr->network.Pump[pump_idx].Schedule[pr->network.Pump[pump_idx].Schedule_idx] = Htime_;
        pr->network.Pump[pump_idx].Schedule_idx++;
    }
}